...
setopt auto_pushd
# cdで存在しないディレクトリを指定した場合、それがスラッシュで始まらない単語であれば、
# 名前付きディレクトリへのcdを試みる（未検討）
#setopt cdable_vars
# ディレクトリスタックに同じものを追加しない
setopt pushd_ignore_dups
# ディレクトリスタックの各要素へのアクセス時の+と-の意味を反転する（未検討・不要？）
#setopt pushd_minus
# pushd, popd時にそのときのディレクトリスタックの内容を非表示（未検討・不要？）
#setopt pushd_silent
# pushdを引数なしで起動したときにホームディレクトリに移動
